...
export async function POST(request: NextRequest) {
  try {
    ...
    const edgeFunctionUrl = `${supabaseUrl}/functions/v1/publish-tiktok`
    ...
    if (!response.ok) {
      ...
      console.error('TikTok edge function error:', errorData)
      return NextResponse.json(
        { error: 'Failed to publish to TikTok', details: errorData },
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Check if episode has been published to TikTok
    ...
    return NextResponse.json({
      ...
      videoUrl: `https://www.tiktok.com/@username/video/${analytics.video_id}`, // Placeholder username
